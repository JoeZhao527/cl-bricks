...
def ensemble_final_result(prob_prediction_paths: dict, output_base: str):
    """
    ...
    final_res.to_csv(os.path.join(output_base, "final_prediction.csv"), index=False)
    ...
    np.save(os.path.join(output_base, "compressed_final_prediction.npy"), np.stack(np.where(arr == 1)))
